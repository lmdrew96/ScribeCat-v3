# Codex Instructions for ScribeCat v3
...
**Tech Stack:** React 19, TypeScript, Tailwind CSS 4 + shadcn/ui, TipTap editor, Excalidraw diagrams, Convex backend, Clerk auth, AssemblyAI transcription, Codex AI
...
| **AI** | Anthropic Codex | Haiku 4.5 (all endpoints, centralized in `convex/config.ts`) |
...
│   ├── parseDocument.ts       # Document/image parsing via Codex Vision
...
- Uses `convex/ai.ts` (Convex action `generateNotesFromTranscript`)
